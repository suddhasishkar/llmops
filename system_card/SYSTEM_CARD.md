...
| Model deployment | real Microsoft Foundry (`AIServices`) `gpt-4o-mini` deployment (`azd`-provisioned; endpoint/deployment name in `azd env get-values`). No stub or offline model path exists in this codebase. One shared deployment serves all three agent roles. | per model version bump | AI/LLMOps team |
