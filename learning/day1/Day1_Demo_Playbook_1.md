...
satisfy. Use a **user-assigned managed identity (UAMI)**, not an app
registration — `az ad app create`/`az ad sp create` need a Microsoft
Entra ID directory role a lot of orgs restrict even for subscription
Owners/Contributors, and fail with `Insufficient privileges to complete
the operation` if you don't have it. A UAMI needs only the same
subscription RBAC `azd provision` already required, so it works
everywhere the app-registration path might not (the app-registration
alternative, for tenants that do allow it, is in `Day1_Lab_Guide.md`
0.9.b′):

```bash
az identity create --name nimbus-eval-cloudeval --resource-group rg-nimbus-eval

az identity federated-credential create \
  --name github-pull-request \
  --identity-name nimbus-eval-cloudeval \
  --resource-group rg-nimbus-eval \
  --issuer 'https://token.actions.githubusercontent.com' \
  --subject 'repo:<org>/<repo>:pull_request' \
  --audiences 'api://AzureADTokenExchange'

PRINCIPAL_ID=$(az identity show --name nimbus-eval-cloudeval --resource-group rg-nimbus-eval --query principalId -o tsv)
...
CLIENT_ID=$(az identity show --name nimbus-eval-cloudeval --resource-group rg-nimbus-eval --query clientId -o tsv)
```
...
| Add | Kind | Value |
|---|---|---|
| `EVAL_AZURE_CLIENT_ID` | secret | `$CLIENT_ID` from Phase 3 |
