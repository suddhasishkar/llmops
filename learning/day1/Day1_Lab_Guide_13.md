...
subject mismatch surfaces as an Azure login failure in the job logs with
`AADSTS700213: No matching federated identity record found for presented
assertion` (not as a GitHub-side error), so if a job's cloud login step
fails, check which trigger produced the run against which federated
credential subject exists before assuming the credential value itself is
wrong.

**Don't assume the plain `repo:<owner>/<repo>:...` format — some repos
present owner and repo names with a numeric ID suffix instead**, e.g.
`repo:someowner@320173705/somerepo@1343697689:ref:refs/heads/main`. Every
federated credential above was written assuming the plain form; if yours
doesn't match, `AADSTS700213` is what you'll see even though the
subject *looks* right at a glance. Don't guess at this — the job log's
"Federated token details" section (printed by `azure/login` right before
the error) shows you the *exact* string GitHub actually asserted, `subject
claim - ...`. Copy that literal string and register a credential against
it directly:
```bash
az identity federated-credential create \
  --name github-main-exact \
  --identity-name <your identity> \
  --resource-group <its resource group> \
  --issuer 'https://token.actions.githubusercontent.com' \
  --subject '<paste the exact "subject claim" value from the job log>' \
  --audiences 'api://AzureADTokenExchange'
```
(swap `az identity federated-credential create` for `az ad app
federated-credential create --id <app-id> --parameters '{...}'` if
you're on the app-registration path instead of a UAMI). An identity can
hold up to 20 federated credentials, so there's no need to delete
whatever's already there — just add the one that actually matches. Fix
this once per identity (the deploy identity for `ref:refs/heads/main`,
the eval identity for `pull_request`) and it's done for every future
...
| `azure/login` fails with `AADSTS700213: No matching federated identity record found for presented assertion subject '...'` even though you registered a federated credential and the subject *looks* right | Some repos present the OIDC subject with a numeric ID suffix on the owner and repo names (`repo:owner@<id>/repo@<id>:...`) instead of the plain `repo:owner/repo:...` form every federated credential in this guide assumes | Don't guess — the job log prints the exact asserted string right above the error, under "Federated token details" → `subject claim - ...`. Copy that literal value and register a new federated credential against it (see the callout in 0.9.a above); an identity can hold up to 20, so add rather than replace |
| `azure/login` fails with `Not all values are present. Ensure 'client-id' and 'tenant-id' are supplied` | The `EVAL_AZURE_*`/`AZURE_*` GitHub secrets the job reads are empty — usually because they were saved as Variables instead of Secrets, saved under a GitHub Environment instead of the repository, or never actually saved | Set them with `gh secret set NAME --body "$(az ... -o tsv)"` (writes a repository-level Secret, sidesteps all three) and confirm with `gh secret list` |
