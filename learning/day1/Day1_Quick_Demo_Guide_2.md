...
14. **Shared eval environment** (this is the piece that costs real money
    per PR while it's on). If you want the demo to show real Langfuse
    traces too — otherwise the gateway's tracing calls just fail
    silently in the background, nothing to point at — set the keys
    **before** `azd up`, not after (a free Langfuse Cloud Hobby project
    at https://cloud.langfuse.com takes under a minute to create):
    ```bash
    azd env new nimbus-eval
    azd env set LANGFUSE_PUBLIC_KEY pk-lf-...   # skip these two lines to demo without Langfuse
    azd env set LANGFUSE_SECRET_KEY sk-lf-...
    azd up
    ```
    If `azd up` fails on the Foundry **project** step with `you must
    enable a managed identity on your resource` even though the account
    clearly has one — that's a real, known Azure propagation race, not
    something you did wrong. `Day1_Lab_Guide.md`'s troubleshooting table
    has the two-phase `DEPLOY_FOUNDRY_PROJECT` workaround for it.

    Create the eval identity — a separate one from step 13's, since it
    needs a federated credential with a different subject (`pull_request`,
    not `main`). Use a **user-assigned managed identity (UAMI)**, not an
    app registration: `az ad app create`/`az ad sp create` need a
    Microsoft Entra ID directory role a lot of orgs don't grant even to
    subscription Owners/Contributors, and if that's you, those commands
    fail with `Insufficient privileges to complete the operation`. A UAMI
    needs only the same subscription RBAC `azd provision` already
    required, so it works everywhere the app-registration path might not
    (full explanation, plus the app-registration alternative if your
    tenant does allow it, is in `Day1_Lab_Guide.md` 0.9.b/0.9.b′; do not
    skip the role grants below, `cloud-eval` will fail without them even
    ...
    SCOPE="/subscriptions/$(az account show --query id -o tsv)/resourceGroups/$EVAL_RG"
    ...
    CLIENT_ID=$(az identity show --name nimbus-eval-cloudeval --resource-group "$EVAL_RG" --query clientId -o tsv)
    ```
    Then in GitHub, **Settings → Secrets and variables → Actions**, add:

    | Add | Kind | Value |
    |---|---|---|
    | `EVAL_AZURE_CLIENT_ID` / `_TENANT_ID` / `_SUBSCRIPTION_ID` | secret | `$CLIENT_ID`, your tenant ID, your subscription ID |
...
16. **Run the actual demo:**
    - Create a branch, set `PROMPT_VERSION: candidate_broken` in
      ...
      locally in step 10, now happening inside GitHub. If you set the
      Langfuse keys in step 14, this is the moment to have your Langfuse
      Cloud project open in another tab — the traces from `cloud-eval`'s
      real model calls land there within a few seconds of each step
      running, so you can show the actual call/latency/cost breakdown
      live instead of just a green checkmark.
